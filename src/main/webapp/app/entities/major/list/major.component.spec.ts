...
import { sampleWithRequiredData } from '../major.test-samples';
...
describe('Major Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.majors?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to majorService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getMajorIdentifier');
      const id = comp.trackId(entity);
      expect(service.getMajorIdentifier).toHaveBeenCalledWith(entity);
