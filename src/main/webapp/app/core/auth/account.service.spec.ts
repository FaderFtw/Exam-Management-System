  ...
  describe('hasAnyAuthority', () => {
    describe('hasAnyAuthority string parameter', () => {
      it('should return false if user is not logged', () => {
        const hasAuthority = service.hasAnyAuthority([Authority.PROFESSOR, Authority.DEPARTMENT, Authority.INSTITUTE, Authority.SUPER]);
        expect(hasAuthority).toBe(false);
      });

      it('should return false if user is logged and has not authority', () => {
        service.authenticate(accountWithAuthorities([Authority.PROFESSOR]));

        const hasAuthority = service.hasAnyAuthority(Authority.SUPER);

        expect(hasAuthority).toBe(false);
      });

      it('should return true if user is logged and has authority', () => {
        service.authenticate(accountWithAuthorities([Authority.INSTITUTE]));

        const hasAuthority = service.hasAnyAuthority(Authority.INSTITUTE);
    ...
    describe('hasAnyAuthority array parameter', () => {
      ...
      it('should return true if user is logged and has authority', () => {
        service.authenticate(accountWithAuthorities([Authority.DEPARTMENT]));
